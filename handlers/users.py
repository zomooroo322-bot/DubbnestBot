...
def register_user_handlers(dp: Dispatcher, bot: Bot):
    ...
    # ── Admin voice Accept callback ───────────────────────────────────────
    @dp.callback_query(F.data.startswith("av_accept:"))
    async def cb_av_accept(callback: CallbackQuery):
        _, admin_id_str, user_tg_str, db_user_id_str = callback.data.split(":")
        admin_id   = int(admin_id_str)
        user_tg_id = int(user_tg_str)
        # Only the tagged admin can accept
        if callback.from_user.id != admin_id:
            return await callback.answer("❌ Only the assigned admin can accept.", show_alert=True)
        ...
        # Edit caption to show accepted
        try:
            old_cap = callback.message.caption or ""
            await callback.message.edit_caption(
                caption=old_cap + f"\n\n✅ <b>Accepted by {admin_name}</b> — deadline: {deadline}",
                parse_mode="HTML"
            )
        except Exception:
            pass
        ...
        try:
            await bot.send_message(user_tg_id,
                f"🎙 <b>Voice Session Accepted!</b>\n\n"
                f"✅ <b>{admin_name}</b> has accepted your voice session request.\n"
                f"📅 They will respond within <b>3 days</b> (by {deadline}).\n\n"
                f"Stay tuned for their message!",
                parse_mode="HTML"
            )
        except Exception:
            pass

    # ── Admin voice Decline callback ──────────────────────────────────────
    @dp.callback_query(F.data.startswith("av_decline:"))
    async def cb_av_decline(callback: CallbackQuery):
        _, admin_id_str, user_tg_str, db_user_id_str = callback.data.split(":")
        admin_id   = int(admin_id_str)
        user_tg_id = int(user_tg_str)
        db_user_id = int(db_user_id_str)
        # Only the tagged admin can decline
        if callback.from_user.id != admin_id:
            return await callback.answer("❌ Only the assigned admin can decline.", show_alert=True)
        admin_name = callback.from_user.first_name or "Admin"
        # Return admins_voices to user inventory
        await add_to_inventory(db_user_id, "admins_voices")
        await execute("UPDATE users SET items_used = items_used - 1 WHERE id = ?", (db_user_id,))
        await callback.message.edit_reply_markup(reply_markup=None)
        ...
        try:
            old_cap = callback.message.caption or ""
            await callback.message.edit_caption(
                caption=old_cap + f"\n\n❌ <b>Declined by {admin_name}</b> — item returned to user.",
                parse_mode="HTML"
            )
        except Exception:
            pass
        # Notify user and return item
        try:
            await bot.send_message(user_tg_id,
                f"🎙 <b>Voice Session Declined</b>\n\n"
                f"❌ <b>{admin_name}</b> is currently unavailable for a voice session.\n\n"
                f"✅ Your <b>admins_voices</b> item has been returned to your inventory.\n"
                f"Use /inv to check — you can try selecting another admin with /use admins_voices.",
                parse_mode="HTML"
            )
        except Exception:
            pass
    ...
    async def service_clip_handler(message: Message):
        ...
        if item == "admins_voices":
            ...
            caption   = (
                ...
                f"🎯 Admin: {admin_tag}\n"
                f"⏰ {datetime.datetime.now().strftime('%Y-%m-%d %H:%M')}\n\n"
                f"⚠️ {admin_tag} — please accept or decline below.\n"
                f"If accepted, respond within <b>3 days</b> (by {deadline}).\n"
                f"#uid_{uid}"
            )
            av_keyboard = InlineKeyboardMarkup(inline_keyboard=[[
                InlineKeyboardButton(
                    text="✅ Accept",
                    callback_data=f"av_accept:{admin_id}:{uid}:{session['user_id']}"
                ),
                InlineKeyboardButton(
                    text="❌ Decline",
                    callback_data=f"av_decline:{admin_id}:{uid}:{session['user_id']}"
                ),
            ]])
        ...
        try:
            send_kwargs = {"caption": caption, "parse_mode": "HTML"}
            if item == "admins_voices":
                send_kwargs["reply_markup"] = av_keyboard
            if message.video:
                await bot.send_video(PURCHASES_LOG_ID, message.video.file_id, **send_kwargs)
            elif message.audio:
                await bot.send_audio(PURCHASES_LOG_ID, message.audio.file_id, **send_kwargs)
            elif message.voice:
                await bot.send_voice(PURCHASES_LOG_ID, message.voice.file_id, **send_kwargs)
            elif message.document:
                await bot.send_document(PURCHASES_LOG_ID, message.document.file_id, **send_kwargs)
            ...
            await message.reply("❌ Something went wrong. Contact an admin directly.")import asyncio
import random
import datetime
from aiogram import Bot, Dispatcher, F
from aiogram.filters import Command
